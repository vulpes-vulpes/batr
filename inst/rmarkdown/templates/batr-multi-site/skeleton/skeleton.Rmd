...
# Build canonical night/date columns used for filtering
if (!"Night" %in% names(observations) && "Timestamp" %in% names(observations)) {
  observations$Night <- as.Date(observations$Timestamp)
}

if (!"Night" %in% names(observations)) {
  stop("observations must contain either a Night or Timestamp column")
}

observations$Night <- as.Date(observations$Night)
if (all(is.na(observations$Night))) {
  stop("No valid dates found in observations$Night")
}
...
# Optional year filter (applies to all report outputs)
if (!is.null(params$report_year) && !all(is.na(params$report_year))) {
  years <- unique(as.integer(params$report_year))
  observations <- observations[lubridate::year(observations$Night) %in% years, ]
}
...
if (exists("active_dates")) {
  ...
  if (!is.null(params$report_year) && !all(is.na(params$report_year)) && "Date" %in% names(active_dates)) {
    active_dates$Date <- as.Date(active_dates$Date)
    years <- unique(as.integer(params$report_year))
    active_dates <- active_dates[lubridate::year(active_dates$Date) %in% years, ]
  }
}
...
# Save filtered data for downstream batr functions so all sections are consistent
filtered_data_path <- tempfile(fileext = ".RData")
if (exists("active_dates")) {
  save(observations, active_dates, file = filtered_data_path)
} else {
  save(observations, file = filtered_data_path)
}
...
batr::species_daily_site_plot(
  ...
  y_scale = params$activity_y_scale,
  text_size = 9,
  date_breaks = "2 weeks"
)
```
...
```{r location_map, fig.cap="Monitoring location map", fig.height=5, fig.width=7, eval=params$include_map}
...
```{r manual_vetting_summary, fig.cap="Manual vetting progress and accuracy by species"}
tryCatch({
  vet_summary <- batr::manual_vet_summary(
    data_path = params$data_path,
    stratify_by = "species",
    species_list = params$species
  )
  
  if (nrow(vet_summary) > 0) {
    kable(vet_summary,
          format = "latex",
          booktabs = TRUE,
          col.names = c("Species", "Total Files", "Vetted Count", "Match Count", "Percent Vetted", "Percent Match"),
          caption = "Manual vetting progress by species") %>%
      kable_styling(latex_options = c("hold_position", "scale_down"))
  } else {
    cat("No manual vetting data available for selected species.\n")
  }
}, error = function(e) {
  cat("Manual vetting summary not available: ", e$message, "\n")
})
```
...
```{r cleanup_temp_data, include=FALSE}
if (exists("filtered_data_path") && file.exists(filtered_data_path)) {
  unlink(filtered_data_path)
}
```
