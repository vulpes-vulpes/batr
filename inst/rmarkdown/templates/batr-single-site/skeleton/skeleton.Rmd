...
if (nrow(unique_locs) > 0) {
  ...
  cat("Location information not available")
}
```

```{r location_map, fig.cap="Monitoring location map", fig.height=5, fig.width=8, eval=params$include_map}
# Create a simple map if ggmap is available
tryCatch({
  if ("Latitude" %in% names(observations) && "Longitude" %in% names(observations)) {
    # Simple plot of coordinates as fallback
    plot(unique(observations$Longitude), 
         unique(observations$Latitude),
  ...
  cat("Map rendering unavailable. Coordinates are provided in the table above.")
})
```
