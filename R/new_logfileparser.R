...
import_logs <- function(log_path, data_path = NULL, monitoring_start = NULL, monitoring_end = NULL) {
  ...
  ###
  wildlife_acoustics_file_list <- list.files(log_path, recursive = TRUE, pattern = "*.txt", full.names = TRUE) # Creates a list of files
  if (length(wildlife_acoustics_file_list) > 1) {
    message("Wildlife Acoustics summary files found. Reading files:")
    logs <- data.table::rbindlist(pbapply::pbsapply(wildlife_acoustics_file_list, data.table::fread, simplify = FALSE, select = c("DATE", "TIME")),
  ...
  rm(wildlife_acoustics_file_list)

  ###
  ### Import Anabat Swift log files and convert to a single neat data.frame for further processing
  ###

  swift_file_list <- list.files(log_path, recursive = TRUE, pattern = "*.csv") # Creates a list of files in the folder
