...
#' Plot Species Activity Data for a Project (Aggregated)
#'
...
#' @family Activity Plots
#'
#' @param dataset Object: a data frame with Species, Night, and Count columns.
#' @param project_name Character, a brief reference phrase (deprecated).
#' @param monitoring_start Character, the date monitoring began (e.g. "2019-01-01").
#' @param monitoring_end Character, the date monitoring ceased (e.g. "2019-12-31").
#' @param gaps Object: a gaps data frame. Defaults to NULL.
#' @param title Logical. If TRUE (default), adds title.
#' @param save_directory Character: if provided, saves .png. Defaults to NULL.
#' @param text_size Numeric: adjusts text size.
#' @param date_label Date format string for month labels.
#'
...
species_site_aggregated_plot <- function(dataset, project_name, monitoring_start, monitoring_end, gaps = NULL,
  ...
  # Create labeller for Ontario bat species (hardcoded - should be dynamic)
  species.labs <- c(
    "Big Brown Bat", "Eastern Red Bat", "Hoary Bat", "Silver-haired Bat", "Myotis Spp.",
  ...
  if (isTRUE(title)) {
    species_site_aggregated_plot <- species_site_aggregated_plot +
      ggplot2::ggtitle("Total Activity of Bats")
  }

  if (!is.null(save_directory)) {
    ggplot2::ggsave(
      paste(save_directory, "/Aggregated_Seasonal_Activity_by_Species.png", sep = ""),
...
#' Plot Activity for All Species at Each Site (Multi-Species Wrapper)
#'
#' \code{multi_species_daily_site_plot} creates plots for all species in dataset.
#'
#' @family Activity Plots
#'
#' @param dataset Object: a data frame with Species, Night columns.
...
#' @param gaps Object: gaps data frame. Defaults to NULL.
#' @param survey_year Character: year filter (deprecated).
#' @param save_directory Character: directory to save plots.
#'
#' @return NULL (plots saved to directory).
#'
#' @export
multi_species_daily_site_plot <- function(dataset, project_name, monitoring_start, monitoring_end,
                                          gaps = NULL, survey_year = NULL, save_directory = NULL) {
  species_list <- dataset$Species
  species_list <- c(
    species_list, "Epfu", "Labo", "Laci", "Lano", "Myle", "Mylu", "Myse",
    "Mysp", "Pesu", "HiF", "HiFrag", "HiLo", "LoFrag", "NOISE", "Social", "NoID"
  )
  species_list <- unique(species_list)
  species_list <- species_list[!is.element(species_list, c(
    "HiF", "HiFrag", "HiLo", "LoF", "noID", "NoID",
    "LoFrag", "Social", "NOISE"
  ))]

  for (i in species_list) {
    old_species_daily_site_plot(
      dataset, project_name, i, monitoring_start, monitoring_end,
      gaps, survey_year, save_directory
    )
  }
}
