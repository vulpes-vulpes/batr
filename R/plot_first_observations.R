...
first_observations_plot <- function(data_path, species, timezone,
  ...
  dataset <- padr::pad(dataset,
    by = "date", start_val = as.Date(monitoring_start),
  ...
  first_observations_plot <- ggplot2::ggplot() +
    ...
    ggplot2::facet_wrap(
      ~Location,
      ncol = 2, scales = "fixed", strip.position = "top"
    ...
    if (!is.null(gaps)) {
      ggplot2::geom_rect(
        data = gaps, ggplot2::aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax, alpha = 0.9),
        show.legend = FALSE
      )
    }
