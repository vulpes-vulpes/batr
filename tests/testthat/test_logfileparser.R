...
test_that(".read_swift_logs parses Anabat Swift files correctly", {
  ...
  file_list <- list.files(td, pattern = "log \\d{4}-\\d{2}-\\d{2}\\.csv$", 
...
test_that(".read_swift_logs detects inactive recorders", {
  ...
  write.table(bad_content, log_file, sep = ",", row.names = FALSE, 
...
test_that(".read_ranger_logs parses Anabat Ranger files correctly", {
  ...
  file_list <- list.files(td, pattern = "ranger.*\\.csv$", 
                          full.names = TRUE, recursive = TRUE)
  expect_true(length(file_list) == 2)
...
test_that("import_logs processes Wildlife Acoustics logs end-to-end", {
  ...
  mock_check <- function(data_path, action, object) {
    if (is.null(data_path)) return(rdata_tmp)
    return(data_path)
  }
  ...
  }, .check_data_path = mock_check, .save_to_RDATA = mock_save)
})
